...
interface Report {
  id: string;
  reporter: string;
  subjectId: string; // "external" | senior:<id> | student:<id>
  subjectLabel: string;
  category: Category;
  severity: Severity;
  description: string;
...
// Lightweight heuristic — suggests severity from free-text keywords.
function suggestSeverity(text: string): Severity {
  const t = text.toLowerCase();
  if (
    /(unsafe|threat|assault|harass|abuse|scam|fraud|stolen|theft|weapon|emergency|danger)/.test(
      t,
    )
  )
    return "high";
  if (/(rude|uncomfortable|inappropriate|late|no.?show|missed|cancel|ignore)/.test(t))
    return "medium";
  return "low";
}
...
const seedFactory = (): Report[] => [
  ...
  },
  {
    ...
    severity: "high",
    description:
      "Mentor asked detailed personal financial questions during the session.",
    status: "open",
...
function ReportPage() {
  const { seniors, students } = useApp();

  const [reports, setReports] = useState<Report[]>(seedFactory);
  const [reporter, setReporter] = useState("");
  const [subjectId, setSubjectId] = useState<string>("external");
  ...
  const subjectOptions = useMemo(
    () => [
      { id: "external", label: "Someone not listed (enter name)" },
      ...seniors.map((s) => ({ id: `senior:${s.id}`, label: `${s.name} · Senior` })),
      ...students.map((s) => ({ id: `student:${s.id}`, label: `${s.name} · Mentor` })),
    ],
    [seniors, students],
  );
  ...
  const applySuggestion = () => {
    ...
    toast.success(`Applied: ${aiCategory} · ${aiSeverity} severity`);
  };

  const submit = () => {
    if (!reporter.trim()) return toast.error("Please enter your name");
    let subjectLabel = "";
    if (subjectId === "external") {
      if (!externalSubject.trim())
        return toast.error("Who is this report about? Enter a name.");
      subjectLabel = externalSubject.trim();
    } else {
      subjectLabel = subjectOptions.find((o) => o.id === subjectId)?.label ?? "Unknown";
    }
    if (description.trim().length < 10)
      return toast.error("Please describe what happened (10+ characters)");
  ...
  const advance = (id: string) => {
    setReports((arr) =>
      arr.map((r) => {
        if (r.id !== id) return r;
        const next: Status =
          r.status === "open" ? "reviewing" : r.status === "reviewing" ? "resolved" : "resolved";
  ...
  const highSeverity = reports.filter((r) => r.severity === "high" && r.status !== "resolved")
    .length;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <Card className="lg:col-span-2 border-border/70 shadow-soft animate-fade-in-up">
          ...
          <CardContent className="space-y-4">
              ...
              <div className="rounded-lg border border-[#014bad]/15 bg-[#014bad]/5 px-3 py-2.5 flex items-start gap-2.5 text-xs">
                ...
                <div className="flex-1">
                  <p className="text-slate-700">
                    Smart suggestion:{" "}
                    <span className="font-semibold text-[#014bad]">{aiCategory}</span> ·{" "}
                    <span className="font-semibold text-[#014bad]">{aiSeverity}</span> severity
                  </p>
                </div>
            ...
            <Button
              onClick={submit}
              className="w-full bg-[#014bad] hover:bg-[#013a8a] text-white font-semibold"
            >
        ...
        <Card className="lg:col-span-3 border-border/70 shadow-soft animate-fade-in-up delay-200">
          ...
          <CardContent>
            <ul className="divide-y divide-border">
                ...
                <li key={r.id} className="py-4 flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span
                        className={`text-[10px] uppercase tracking-wide font-bold px-2 py-0.5 rounded-md ring-1 ${sevColor[r.severity]}`}
                      >
                        {r.severity}
                      </span>
                      <span
                        className={`text-[10px] uppercase tracking-wide font-bold px-2 py-0.5 rounded-md ring-1 ${statusColor[r.status]}`}
                      >
                        {r.status}
                      </span>
                    </div>
              ...
              {filtered.length === 0 && (
                <li className="py-10 text-sm text-muted-foreground text-center">
  ...
  return (
    <Card className="border-border/70 shadow-soft hover-lift">
      <CardContent className="p-5 flex items-center gap-4">
        <div
          className={`inline-flex h-12 w-12 items-center justify-center rounded-xl ring-1 ${tones[tone]}`}
        >
